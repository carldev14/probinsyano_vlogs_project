    ...
    <main className="flex justify-center items-center h-auto flex-col">
      ...
      <section className="flex items-center gap-3 flex-wrap justify-center">
        {linksData.map((item) => {
          if (item.type === "fbpage") {
            return (
              <a href={item.url} target="_blank" className="p-2 rounded-lg  text-xs text-white bg-blue-500">
                {item.name}
              </a>
            )
          } else {
            return (
              <Link href={item.url} className="p-2 rounded-lg  text-xs text-white bg-blue-500">
                {item.name}
              </Link>
            )
          }
